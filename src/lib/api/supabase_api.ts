...
export const getRestaurantsWithName = async (names: string[]) => {
  try {
    ...
    const { data, error } = await supabase
      .from('places')
      .select('*')
      .in('category_group_code', ['FD6', 'CE7'])
      .in('place_name', names);
...
export const searchRestaurantwithName = async (
  keyword: string,
  ...
  try {
    ...
    const { data, error } = await query.ilike(
      'place_name',
      `%${keyword.trim()}%`,
    );
...
export const getRestaurantsWithPagination = async (
  page: number,
  dataPerPage: number,
) => {
  try {
    ...
    const { data, error } = await supabase
      .from('places')
      .select('*')
      .eq('category_group_code', 'FD6')
      .range(page * dataPerPage, (page + 1) * dataPerPage - 1);
...
export const getBookmarks = async (userId: string) => {
  try {
    ...
    const { data, error } = await supabase
      .from('call_bookmarks_with_places')
      .select(
        'id,address_name,category_group_code,category_group_name,category_name,latitude,longitude,phone,place_name,place_url,road_address_name',
      )
      .eq('user_id', userId);
...
export const getPlacesWithUserBookmarks = async (
  userId: string,
  categoryFilter?: string,
) => {
  const query = supabase.rpc('get_places_with_bookmarks', {
    p_user_id: userId,
  });
...
export const getPlacesWithNameAndBookmarks = async (
  userId: string,
  names: string[],
  categoryFilter?: string,
) => {
  const query = supabase.rpc('get_places_with_name_and_bookmarks', {
    p_place_names: names,
    p_user_id: userId,
  });
...
export const getUserCalendar = async (userId: string) => {
  try {
    ...
    if (error) {
      console.error('Supabase error in getUserCalendar:', error);
      throw error;
    }
    console.log(`Found ${data?.length} calendar entries`);
    return data;
  } catch (error) {
...
export const setUserCalendar = async (userId: string, calendarId: string) => {
  try {
    if (!supabase) {
      throw new Error('Supabase client not initialized');
    }
    ...
    if (error) {
      console.error('Supabase error in setUserCalendar:', error);
      throw error;
    }
    return data;
  } catch (error) {
    console.error('Error in setUserCalendar:', error);
    return null;
  }
};

// 해시태그(카테고리/태그) 기반 검색: #카페 한강 → 카테고리에 '카페' 포함 AND 상호명 '한강' 포함
export const searchRestaurantsByTag = async (
  tag: string,
  nameKeyword?: string,
) => {
  try {
    ...
    query = query.or(
      `category_name.ilike.%${trimmedTag}%,category_group_name.ilike.%${trimmedTag}%`,
    );
